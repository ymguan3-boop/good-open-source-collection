...
def write_outputs(out: Path, scope: Any, result: gpd.GeoDataFrame, stats: dict[str, Any], sources: list[dict[str, Any]], substitutions: list[str], input_facility_count: int, input_count_by_type: dict[str, int]) -> None:
    ...
    project = {
        ...
        "layers": [
            ...
            {"id": "yilan-public-facility-exposure", "name": "公共設施複合災害暴露查核結果", "type": "geojson", "source": {"type": "geojson"}, "geojson": result_payload, "visible": True, "opacity": 1, "style": {"fillColor": "#dc2626", "fillOpacity": 0.9, "strokeColor": "#ffffff", "strokeWidth": 1, "circleRadius": 6}, "metadata": {"resultCount": stats["result_count"], "resultUrl": result_url, "analysisRole": "SCREENING_REFERENCE", "popupFields": ["facility_name", "facility_type", "facility_source", "matched_hazard_groups", "matched_hazard_details", "screening_priority", "analysis_predicate"]}},
            {"id": "yilan-sensitive-schools", "name": "位於地質敏感區內的學校（點位）", "type": "geojson", "source": {"type": "geojson"}, "geojson": {"type": "FeatureCollection", "features": sensitive_school_features}, "visible": True, "opacity": 1, "style": {"fillColor": "#facc15", "fillOpacity": 1, "strokeColor": "#111827", "strokeWidth": 2, "circleRadius": 8}, "metadata": {"resultCount": len(sensitive_school_ids), "uniqueInstitutions": len(sensitive_school_institutions), "listUrl": f"{PAGES_ROOT}analysis/{TASK_ID}/sensitive-schools.csv", "geojsonUrl": sensitive_school_url, "predicate": "school representative point intersects official geological-sensitive-area polygon"}},
            {"id": "yilan-county-boundary", "name": "宜蘭縣行政界", "type": "geojson", "source": {"type": "geojson"}, "geojson": scope_payload, "visible": True, "opacity": 0.45, "style": {"fillColor": "#64748b", "fillOpacity": 0.05, "strokeColor": "#334155", "strokeWidth": 2}, "metadata": {"source": URLS["county_boundary_dataset"], "geojsonUrl": scope_url, "role": "context"}},
        ],
    ...
    (out / "index.html").write_text("<!doctype html><meta charset='utf-8'><title>宜蘭縣公共設施複合災害暴露查核</title><script>location.replace('" + PAGES_ROOT + "?locale=zh-TW&url=' + encodeURIComponent('" + project_url + "'))</script>", encoding="utf-8")
